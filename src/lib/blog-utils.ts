...
// [의정부 시민 포털 정체성에 맞춘 7대 무기 정규식 패턴]
const KEY_POINT_PATTERNS = /^(?:#+\s*)?(?:[💡⭐📌\s]*)(?:시정\s*핵심\s*요약|핵심\s*요약|핵심\s*포인트|요약|3줄\s*요약|시정\s*브리핑|Key\s*Points)(?:[\s:]*)/i;
const CHECKLIST_PATTERNS = /^(?:#+\s*)?(?:[📋✅☑️\s]*)(?:신청\s*자격\s*1분\s*자가진단|1분\s*자가진단|자가진단|체크리스트|Checklist|필수\s*서류\s*체크|자격\s*체크)(?:[\s:]*)/i;
