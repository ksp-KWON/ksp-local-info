...
const KEY_POINT_PATTERNS = /^(?:#+\s*)?(?:[💡⭐📌\s]*)(?:시정\s*핵심\s*요약|행정\s*핵심\s*요약|핵심\s*요약|핵심\s*포인트|요약|3줄\s*요약|시정\s*브리핑|공연\s*핵심\s*요약|Key\s*Points)(?:[\s:]*)/i;
...
const INSIGHT_PATTERNS = /^(?:#+\s*)?(?:[🧭💡📌\s]*)(?:의정부\s*생활\s*꿀팁\s*&\s*행정\s*인사이트|의정부\s*생활\s*꿀팁\s*&\s*시정\s*인사이트|의정부\s*생활\s*꿀팁|행정\s*인사이트|시정\s*인사이트|알짜\s*혜택\s*팁|생활\s*꿀팁)(?:[\s:]*)/i;
