...
export default function EmergencyBottomSheet({ item, activeTab, onClose }: BottomSheetProps) {
  ...
  return (
    <div className="absolute z-10 bottom-6 left-4 right-4 max-w-md mx-auto animate-slide-up">
      <div className="bg-white/95 dark:bg-[#1a1c20]/95 backdrop-blur-md rounded-none overflow-hidden flex flex-col shadow-2xl border border-gray-100 dark:border-gray-800">
        {/* Header */}
      ...
      <div className="p-6 space-y-5">
          {activeTab === 'er' && (
            <div className="flex items-center justify-between">
              ...
              <span className={`px-3 py-1.5 rounded-full text-sm font-bold shadow-2xl ${item.status === 'good' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : item.status === 'busy' ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
          ...
          <div className="mt-2">
            <a 
              ...
              className={`flex items-center justify-center w-full py-3.5 rounded-none font-bold text-white transition-all shadow-2xl hover:shadow-2xl hover:-translate-y-0.5 ${activeTab === 'er' ? 'bg-red-500 hover:bg-red-600' : 'bg-emerald-600 hover:bg-emerald-700'}`}
            >
              길찾기 바로가기
            </a>
          </div>
