...
export default function AdminDashboardClient() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-gray-50 dark:bg-zinc-950 font-sans h-full overflow-hidden relative">
      {message && (
        <div className={`fixed top-4 right-4 z-50 p-4 rounded-none shadow-2xl transition-all transform flex items-center gap-3 ${
      ...
      <nav className="h-14 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between px-4 sm:px-6 shrink-0 shadow-2xl z-40">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-600 rounded-none flex items-center justify-center text-white font-bold text-lg shadow-2xl">
            🏥
          </div>
        ...
        <div className="hidden md:flex w-[240px] shrink-0 flex-col bg-white dark:bg-zinc-900 border-r border-gray-200 dark:border-zinc-800 p-4 overflow-y-auto custom-scrollbar z-20">
          <div className="space-y-1">
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-none text-sm font-bold transition-all ${activeTab === 'hospitals' ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 shadow-2xl' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'}`}
            >
              🏥 야간·휴일 진료 병원
            </button>
            <button
              onClick={() => setActiveTab('events')}
              className={`w-full text-left px-4 py-3 rounded-none text-sm font-bold transition-all ${activeTab === 'events' ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 shadow-2xl' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'}`}
            >
              🎪 행사·축제 정보
            </button>
            <button
              onClick={() => setActiveTab('benefits')}
              className={`w-full text-left px-4 py-3 rounded-none text-sm font-bold transition-all ${activeTab === 'benefits' ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 shadow-2xl' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'}`}
            >
              🎁 지원금·혜택 정보
            </button>
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-none text-sm font-bold transition-all ${activeTab === 'posts' ? 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 shadow-2xl' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'}`}
            >
              📝 블로그 포스트 관리
            </button>
            <button
              onClick={() => setActiveTab('settings')}
              className={`w-full text-left px-4 py-3 rounded-none text-sm font-bold transition-all ${activeTab === 'settings' ? 'bg-slate-200 text-slate-800 dark:bg-zinc-800 dark:text-slate-200 shadow-2xl' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800'}`}
            >
              ⚙️ 환경 설정
            </button>
          </div>
