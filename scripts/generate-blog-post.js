...
- 가독성이 좋아지도록 중요한 키워드는 **굵은 글씨(볼드체)**로 강조해줘.
- 단, 마크다운 표준(CommonMark) 규격 준수를 위해, **강조할 단어** 뒤에 조사(예: ~은, ~는, ~이, ~가, ~을, ~를, ~으로, ~에 등)가 올 경우 조사를 강조 표시 안에 포함시켜 작성해줘. (예: **혜택을** [O] / **혜택**을 [X])
- 한글 특수문자나 문장 부호, 혹은 괄호(예: 「」, ~, ())와 인접하여 강조 표시를 사용할 경우에도, 강조 표시(**)의 앞뒤로 공백을 최소 한 칸 띄우고 작성하여 표준 마크다운 파서가 정상적으로 굵은 글씨를 인식할 수 있게 해줘.
